...
1. レイアウト構築後、次を読み込んで初期表示する。
   ...
   - **フォーム入力の復元**: 抽出条件・除外条件・先頭付加・末尾付加・拡張子追加文字を、それぞれ `pattern.txt` / `exclude-pattern.txt` / `clipboard-prefix.txt` / `clipboard-suffix.txt` / `file-suffix.txt`（いずれも `~/.filecollector/` 下）から読み、該当入力欄に表示する（ファイルが無い項目はデフォルトのまま）。
...
3. ファイル名は「拡張子追加文字」を付与したうえで、**同名が 2 件以上ある場合のみ `名前(1).ext` / `名前(2).ext` のように連番を付与して重複を避ける**。同名が 1 件だけの場合はそのままのファイル名で出力する。
...
2. **フォーム入力の保存**: 抽出条件・除外条件・先頭付加・末尾付加・拡張子追加文字を、上記と同じ `~/.filecollector/` 下の各ファイルに上書き保存する。
...
| **文字付加する（チェックボックス）** | ON のときのみ「先頭付加・末尾付加」の文字列を適用します。OFF のときはファイル内容のみをそのまま結合して出力します。 |
| **クリップボードに出力** | 抽出結果リストで選択したファイルの内容を、必要に応じて先頭・末尾付加を適用して結合し、クリップボードにコピーします。選択時のみ有効。 |
...
`FileCollector.groovy` は **FileCollector**（エントリ）と **FileCollectorFrame**（メインウィンドウ）の 2 クラスで構成される。メソッドは役割ごとに次のように分かれる。
...
| `loadFormSettings()` | 抽出条件・除外条件・先頭付加・末尾付加・拡張子追加文字をファイルから読み、各入力欄に復元する。 |
| `saveFormSettings()` | 上記フォーム入力を各ファイルに保存する。 |
...
| メソッド | 説明 |
|----------|------|
| `fileNameWithSuffix(fileName, suffix)` | ファイル名の末尾に拡張子追加文字を付与する。 |
